...
from langchain.text_splitter import RecursiveCharacterTextSplitter  # 청킹 도구
...
# OpenAIEmbeddings: OpenAI의 text-embedding-ada-002 모델을 사용
# 각 텍스트를 1536차원(1536개 숫자)의 벡터로 변환한다
# OPENAI_API_KEY 환경변수가 설정되어 있어야 한다
os.environ["OPENAI_API_KEY"] = "sk-..."  # 실제 키를 입력하세요
...
# RetrievalQA: 검색(Retrieval) + 답변생성(QA)을 하나로 묶은 체인
# chain_type="stuff": 검색된 문서를 모두 합쳐서 LLM에 전달하는 방식
# return_source_documents=True: 어떤 문서를 참고했는지도 반환
qa_chain = RetrievalQA.from_chain_type(
    llm=llm,
    chain_type="stuff",               # 문서를 그대로 채워 넣는 방식
    retriever=retriever,              # 앞에서 만든 검색기
    return_source_documents=True,     # 참고 문서 목록도 반환
    chain_type_kwargs={"prompt": prompt_template}  # 커스텀 프롬프트 사용
)
...
for i, source_doc in enumerate(result['source_documents'], 1):
    print(f"  [{i}] {source_doc.metadata.get('source', '알 수 없음')}")
    print(f"      {source_doc.page_content[:80]}...")
```

!!! note "코드 설명"
    - `ChatOpenAI(temperature=0)`: 답변의 무작위성을 최소화. 사실 기반 답변에 적합
    - `PromptTemplate`: LLM에게 "참고 문서만 보고 답해라"고 명확히 지시
    - `RetrievalQA`: 검색 + 생성을 한 번에 처리하는 편리한 체인
    - `return_source_documents=True`: 어떤 문서를 참고했는지 추적 가능 (신뢰성 향상)
...
os.environ["OPENAI_API_KEY"] = "여기에_실제_API_키를_입력하세요"

embeddings = OpenAIEmbeddings(model="text-embedding-3-small")
vectorstore = Chroma.from_documents(chunks, embeddings)
print(f"[3단계] 벡터 DB 저장 완료: {vectorstore._collection.count()}개 벡터")
...
for question in questions:
    ...
    print(f"\n참고 문서 수: {len(result['source_documents'])}개")
```
