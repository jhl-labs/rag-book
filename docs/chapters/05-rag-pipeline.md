...
print(f"벡터 DB 저장 완료!")
print(f"저장된 청크 수: {len(chunks)}")  # 참고: _collection은 내부 API
...
print(f"기존 DB 불러오기 완료: {vectorstore_loaded.get()['ids'].__len__()}개 청크")
```
...
!!! warning "대화형 RAG의 검색 한계"
    위 구현에서는 "그거 어떤 장점이 있어?" 같은 대명사가 포함된 질문을
    그대로 벡터 검색에 사용합니다. "그거"로는 관련 문서를 찾기 어렵습니다.

    프로덕션에서는 LangChain의 `create_history_aware_retriever`를 사용하여
    대화 기록을 참고해 검색 쿼리를 자동 재작성하는 것이 권장됩니다.
...
if Path(DB_DIR).exists():
    ...
    print(f"  {len(vectorstore.get()['ids'])}개 청크 로드됨")  # 참고: _collection은 내부 API
else:
    ...
    print(f"  {len(chunks)}개 청크 저장됨")  # 참고: _collection은 내부 API
